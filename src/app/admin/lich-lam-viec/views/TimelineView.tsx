...
const TimelineView: React.FC<TimelineViewProps> = ({
  ...
  const getShiftInfo = (shiftName: string) => {
    const shiftMap: Record<string, { icon: any; color: string; label: string }> = {
      "Ca sáng": { icon: <ClockCircleOutlined />, color: "#52c41a", label: "Sáng" },
      "Ca chiều": { icon: <ClockCircleOutlined />, color: "#1890ff", label: "Chiều" },
      "Ca tối": { icon: <ClockCircleOutlined />, color: "#fa8c16", label: "Tối" },
      "Ca đêm": { icon: <ClockCircleOutlined />, color: "#722ed1", label: "Đêm" },
    };
    return shiftMap[shiftName] || { icon: <ClockCircleOutlined />, color: "#d9d9d9", label: shiftName };
  };
    ...
    <div className="timeline-schedule-view" style={{ padding: "20px" }}>
      ...
      <div style={{ marginBottom: "24px" }}>
        ...
        <Text type="secondary" style={{ fontSize: "14px" }}>
          Từ {start.format("DD/MM/YYYY")} đến {end.format("DD/MM/YYYY")} • {filteredSchedules.length} ca làm việc
        </Text>
      </div>
        ...
        {sortedDates.length === 0 ? (
          ...
          <Timeline mode="left" style={{ paddingTop: "20px" }}>
            {sortedDates.map((date) => {
              ...
              const formattedDate = `${getDayNameInVietnamese(dateObj)}, ${dateObj.format("DD/MM/YYYY")}`;
              ...
              return (
                ...
                >
                  ...
                  <div style={{ marginBottom: "16px" }}>
                    <Title level={4} style={{ margin: 0, color: isToday ? "#52c41a" : "#1890ff" }}>
                      {formattedDate}
                      {isToday && (
                        <Badge
                          count="Hôm nay"
                  ...
                  <Row gutter={[12, 12]}>
                    {Object.entries(shiftGroups).map(([shiftName, shiftSchedules]) => {
                      ...
                      return (
                        <Col key={shiftName} xs={24} lg={12} xl={12}>
                          ...
                          >
                            ...
                            >
                              <span style={{ color: shiftInfo.color, marginRight: "6px" }}>
                                {shiftInfo.icon}
                              </span>
